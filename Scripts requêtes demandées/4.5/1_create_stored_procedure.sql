...
BEGIN
    ...
    -- Traitement des parcelles (si présentes)
    IF p_cad_parcelles IS NOT NULL THEN
        -- Boucle sur chaque parcelle (séparées par '|')
        ...
        LOOP
            ...
            -- Si la table parcelle existait déjà, récupérer son id
            IF v_id_parcelle IS NULL THEN
                SELECT id_parcelle INTO v_id_parcelle 
                FROM parcelle 
                WHERE cad_parcelles = v_parcelle;
            END IF;
